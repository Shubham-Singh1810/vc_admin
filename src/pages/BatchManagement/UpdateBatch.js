...
function UpdateBatch() {
  ...
  const fetchBatchDetails = async () => {
    try {
      const response = await getBatchDetailsServ(id);
  ...
  try {
    ...
    const response = await updateBatchServ(fd); 
  ...
  return (
    <div className="container-fluid p-4">
      <h5 className="mb-3">Update Batch</h5>
      <Formik
      ...
      >
        {({ setFieldValue, values }) => (
          <Form>
            ...
            >
              {submitBtnLoader ? "Updating Batch..." : "Update Batch"}
            </button>
          </Form>
        )}
      </Formik>
