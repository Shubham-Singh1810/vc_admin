...
import {deleteBatchServ, getBatchServ} from "../../services/batch.services"
...
function OngoingBatch() {
  ...
  const handleDeleteFunc = async (id) => {
    try {
      let response = await deleteBatchServ(deleteId);
      if (response?.data?.statusCode == "200") {
        getListFunc();
  ...
  return (
    <div className="container-fluid user-table py-3">
                  ...
                  : list?.map((v, i) => {
                      return (
                        <tr>
                          ...
                          <td style={{ textAlign: "center" }}  onClick={()=>alert("Work in progress")}><u>View</u></td>
      ...
      <ConfirmDeleteModal
        ...
        title="Batch Delete"
        body="Do you really want to delete this Batch?"
      />
    </div>
  );
}
