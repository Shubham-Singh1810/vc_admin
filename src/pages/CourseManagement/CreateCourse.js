...
import { addCourseServ } from "../../services/course.services";
...
function CreateCourse() {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = await addCourseServ(fd);
  ...
  const batchSections = [
    {
      ...
      fields: [
        { label: "Course Image", name: "image", type: "file", required: true },
        ...
        {({ setFieldValue, values }) => (
          <Form>
            ...
            <button type="submit" className="btn bgThemePrimary w-100 text-white" disabled={submitBtnLoader}>
              {submitBtnLoader ? "Creating Course..." : "Create Course"}
            </button>
          </Form>
        )}
      </Formik>
